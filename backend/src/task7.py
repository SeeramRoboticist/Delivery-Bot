...
class deliver7:
    ...
    def confirmation_cb(self, confirmation):
        ...
        if confirmation.data == "confirm_table":
            ...
            self.confirmation_counter.append(self.confirm_table)
            print(self.confirmation_counter)
    ...
    def table(self):
        ...
        """
        Table State: In this State, robot
        will deliver to multiple tables,
        if delivered to all tables, 
        robot will retrun to home, else it
        will go to kitchen and return to 
        home.
        """

        # Reseting delivered Counter
        self.confirmation_counter = []
        
        #Deliver in order of the list

        for i in self.order_list:

            result = self.navigate_with_postion("table" + str(i))

            # When ever he result is done, the robot has reached the table,
            # we wait for some time and confirmation else move on.
            # if result is not done (i.e order cancelled)
            # we we simply navigate to the next location in queue.

            if result == "done":
                self.wait_and_confirmation()


        if len(self.confirmation_counter) == len(self.order_list):
            self.navigate_with_postion("home")
            self.current_process = "home"
            return
    ...
    def wait_and_confirmation(self):

        """
        Here the robot will wait for a 
        certarin time and exit or if 
        confirmed it will exit.
        """

        wait_time = time.time() + self.wait_for
        
        # Resetting flag
        self.confirm_table = False    
...
if __name__ == "__main__":
    ...
    """
    rospy.init_node("task7", anonymous=True)
    obj = deliver7()
    obj.main_process()